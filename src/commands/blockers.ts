...
import { calculateBlockers } from '../core/metrics.js';
import { formatBlockers } from '../core/formatters.js';

export async function blockers(options: BaseOptions = {}): Promise<void> {
  try {
    ...
    // Calculate blockers
    const result = calculateBlockers(commits, startDate, endDate);

    // Output
    if (options.json) {
      console.log(JSON.stringify(result, null, 2));
    } else {
      console.log(formatBlockers(result));
    }
  } catch (error) {
