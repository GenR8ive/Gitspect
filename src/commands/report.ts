...
import { calculateProjectHealth } from '../core/metrics.js';
import { formatProjectHealth } from '../core/formatters.js';

export async function report(options: BaseOptions = {}): Promise<void> {
  try {
    ...
    const health = calculateProjectHealth(commits, startDate, endDate);

    // Output
    if (options.json) {
      console.log(JSON.stringify(health, null, 2));
    } else {
      console.log(formatProjectHealth(health));
    }
  } catch (error) {
